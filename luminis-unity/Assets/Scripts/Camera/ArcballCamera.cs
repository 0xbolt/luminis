...
//             targetToCamera = q * targetToCamera;
//             transform.position = target.position + targetToCamera;
...
//     Vector2 XyToUv(Vector2 deltaPos, Vector3 mousePos) {
//         Vector2 screen = new Vector2(Screen.width, Screen.height);
//         Vector2 p = (new Vector2(mousePos.x, mousePos.y) - screen / 2f) / (radius * screen.y / 2f);
//         float u = Vector2.Dot(p, deltaPos) / p.magnitude;
//         float v = deltaPos - u;
